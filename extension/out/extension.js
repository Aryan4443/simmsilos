...
var require_api = __commonJS({
  "out/api.js"(exports2) {
    ...
    function baseUrl() {
      return vscode2.workspace.getConfiguration("simmsilos").get("apiUrl", "http://localhost:3000");
    }
...
var require_auth = __commonJS({
  "out/auth.js"(exports2) {
    ...
    exports2.ensureFreshToken = ensureFreshToken;
    var vscode2 = __importStar2(require("vscode"));
    var api2 = __importStar2(require_api());
    var TOKEN_KEY = "simmsilos.token";
    async function login(context) {
      const username = await vscode2.window.showInputBox({ prompt: "SimmSilos username" });
      if (!username)
        return;
      const password = await vscode2.window.showInputBox({ prompt: "Password", password: true });
      if (!password)
        return;
      try {
        const { token } = await api2.login(username, password);
        await context.secrets.store(TOKEN_KEY, token);
        vscode2.window.showInformationMessage(`Logged in as ${username}`);
        return token;
      } catch {
        vscode2.window.showErrorMessage("Login failed \u2014 check your credentials");
      }
    }
    async function logout(context) {
      ...
      if (token) {
        try {
          await api2.logout(token);
        } catch {
        }
      }
      await context.secrets.delete(TOKEN_KEY);
      vscode2.window.showInformationMessage("Logged out of SimmSilos");
    }
    ...
    async function ensureFreshToken(context) {
      ...
      if (!token)
        return;
      try {
        const me = await api2.getMe(token);
        if (me.should_refresh) {
          const refreshed = await api2.refreshToken(token);
...
// out/tasksProvider.js
var require_tasksProvider = __commonJS({
  "out/tasksProvider.js"(exports2) {
    "use strict";
    var __createBinding2 = exports2 && exports2.__createBinding || (Object.create ? (function(o, m, k, k2) {
      if (k2 === void 0) k2 = k;
      var desc = Object.getOwnPropertyDescriptor(m, k);
    ...
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.TaskItem = exports2.TasksProvider = void 0;
    var vscode2 = __importStar2(require("vscode"));
    var TasksProvider = class {
      ...
      getChildren() {
        if (!this.tasks.length) {
          return [new TaskItem("No tasks assigned", "", vscode2.TreeItemCollapsibleState.None)];
        }
        return this.tasks.map((t) => new TaskItem(t.function, t.status, vscode2.TreeItemCollapsibleState.None, t));
      }
    };
    exports2.TasksProvider = TasksProvider;
    var TaskItem = class extends vscode2.TreeItem {
      constructor(label, status, collapsible, task) {
        ...
        this.tooltip = task ? `${task.function}
Status: ${task.status}
Branch: ${task.branch}
Assigned: ${new Date(task.assigned_at * 1e3).toLocaleString()}` : "";
        this.contextValue = task ? "task" : "";
        this.iconPath = new vscode2.ThemeIcon(status === "done" ? "check" : status === "in_progress" ? "sync~spin" : "circle-outline");
        if (task) {
          this.command = {
    ...
    exports2.TaskItem = TaskItem;
  }
});

// out/language.js
var require_language = __commonJS({
  "out/language.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
...
// out/siloFs.js
var require_siloFs = __commonJS({
  "out/siloFs.js"(exports2) {
    "use strict";
    var __createBinding2 = exports2 && exports2.__createBinding || (Object.create ? (function(o, m, k, k2) {
      if (k2 === void 0) k2 = k;
      var desc = Object.getOwnPropertyDescriptor(m, k);
    ...
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.SiloFileSystemProvider = exports2.SCHEME = void 0;
    exports2.siloUri = siloUri;
    var vscode2 = __importStar2(require("vscode"));
    var api2 = __importStar2(require_api());
    var language_1 = require_language();
    exports2.SCHEME = "simmsilos";
    var SiloFileSystemProvider = class {
      constructor(getToken) {
        this.getToken = getToken;
        this._onDidChangeFile = new vscode2.EventEmitter();
      ...
      stat(_uri) {
        return { type: vscode2.FileType.File, ctime: 0, mtime: Date.now(), size: 0 };
      }
      readDirectory() {
      ...
      async readFile(uri) {
        ...
        if (!token)
          throw vscode2.FileSystemError.NoPermissions("Not logged in");
        const { content } = await api2.readFile(token, uri.path.replace(/^\//, ""));
        const bytes = Buffer.from(content, "utf8");
        setTimeout(async () => {
          const doc = vscode2.workspace.textDocuments.find((d) => d.uri.toString() === uri.toString());
          if (doc)
            await vscode2.languages.setTextDocumentLanguage(doc, (0, language_1.detectLanguage)(uri.path));
        }, 100);
        return bytes;
      }
      async writeFile(uri, content) {
        ...
        if (!token)
          throw vscode2.FileSystemError.NoPermissions("Not logged in");
        await api2.writeFile(token, uri.path.replace(/^\//, ""), Buffer.from(content).toString("utf8"));
        this._onDidChangeFile.fire([{ type: vscode2.FileChangeType.Changed, uri }]);
        vscode2.window.setStatusBarMessage(`SimmSilos: saved ${uri.path}`, 3e3);
      }
    };
    exports2.SiloFileSystemProvider = SiloFileSystemProvider;
    function siloUri(path) {
      const normalPath = path.startsWith("/") ? path : `/${path}`;
      return vscode2.Uri.parse(`${exports2.SCHEME}://${normalPath}`);
    }
  }
});

// out/filesProvider.js
var require_filesProvider = __commonJS({
  "out/filesProvider.js"(exports2) {
    "use strict";
    var __createBinding2 = exports2 && exports2.__createBinding || (Object.create ? (function(o, m, k, k2) {
      if (k2 === void 0) k2 = k;
      var desc = Object.getOwnPropertyDescriptor(m, k);
    ...
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.FilesProvider = void 0;
    var vscode2 = __importStar2(require("vscode"));
    var api2 = __importStar2(require_api());
    var siloFs_12 = require_siloFs();
    var FilesProvider = class {
      ...
      async getChildren(item) {
        ...
        try {
          ...
          const data = await api2.listFiles(token, path);
          if (data.type === "file")
            return [];
          return data.entries.map((e) => new FileItem(e.name, e.path, e.type));
        } catch (err) {
          const msg = err?.response?.status === 404 ? "No branch assigned \u2014 ask admin" : "Silo service unavailable \u2014 partner service not running";
          return [new FileItem(msg, "", "file")];
        }
      }
    };
    exports2.FilesProvider = FilesProvider;
    var FileItem = class extends vscode2.TreeItem {
      constructor(label, resourcePath, type) {
        super(label, type === "dir" ? vscode2.TreeItemCollapsibleState.Collapsed : vscode2.TreeItemCollapsibleState.None);
        this.resourcePath = resourcePath;
        this.iconPath = new vscode2.ThemeIcon(type === "dir" ? "folder" : "file");
        this.contextValue = type;
        if (type === "file") {
          this.command = {
            command: "vscode.open",
            title: "Open File",
            arguments: [(0, siloFs_12.siloUri)(resourcePath)]
          };
        }
      }
    };
...
// out/loginPanel.js
var require_loginPanel = __commonJS({
  "out/loginPanel.js"(exports2) {
    "use strict";
    var __createBinding2 = exports2 && exports2.__createBinding || (Object.create ? (function(o, m, k, k2) {
      if (k2 === void 0) k2 = k;
      var desc = Object.getOwnPropertyDescriptor(m, k);
    ...
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.LoginPanel = void 0;
    var vscode2 = __importStar2(require("vscode"));
    var LoginPanel = class _LoginPanel {
      static show(context, onLogin) {
        if (_LoginPanel.currentPanel) {
          _LoginPanel.currentPanel._panel.reveal();
          return;
        }
        const panel = vscode2.window.createWebviewPanel("simmsilosLogin", "SimmSilos \u2014 Login", vscode2.ViewColumn.One, { enableScripts: true });
        _LoginPanel.currentPanel = new _LoginPanel(panel, context, onLogin);
      }
        ...
        this._panel.webview.onDidReceiveMessage(async (msg) => {
          if (msg.command === "login") {
            ...
            if (ok) {
              ...
              setTimeout(() => this._panel.dispose(), 1e3);
            } else {
...
<body>
  ...
  <script>
    ...
    exports2.LoginPanel = LoginPanel;
  }
});

// out/welcomePanel.js
var require_welcomePanel = __commonJS({
  "out/welcomePanel.js"(exports2) {
    "use strict";
    var __createBinding2 = exports2 && exports2.__createBinding || (Object.create ? (function(o, m, k, k2) {
      if (k2 === void 0) k2 = k;
      var desc = Object.getOwnPropertyDescriptor(m, k);
    ...
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.WelcomePanel = void 0;
    var vscode2 = __importStar2(require("vscode"));
    var WelcomePanel = class _WelcomePanel {
      static update(data) {
        if (_WelcomePanel._panel) {
          _WelcomePanel._panel.webview.html = getHtml(data);
        }
      }
      static show(_context, data) {
        if (_WelcomePanel._panel) {
          _WelcomePanel._panel.webview.html = getHtml(data);
          _WelcomePanel._panel.reveal();
          return;
        }
        const panel = vscode2.window.createWebviewPanel("simmsilosWelcome", "SimmSilos \u2014 Dashboard", vscode2.ViewColumn.One, { enableScripts: true });
        _WelcomePanel._panel = panel;
        panel.onDidDispose(() => {
          _WelcomePanel._panel = void 0;
        });
        panel.webview.html = getHtml(data);
        panel.webview.onDidReceiveMessage((msg) => {
          if (msg.command === "openTasks") {
            vscode2.commands.executeCommand("simmsilos.tasks.focus");
          } else if (msg.command === "openFiles") {
            vscode2.commands.executeCommand("simmsilos.files.focus");
          } else if (msg.command === "refresh") {
            _WelcomePanel.onRefresh?.();
          }
        });
      }
    };
    exports2.WelcomePanel = WelcomePanel;
    function getHtml(data) {
      const taskRows = data.tasks.length ? data.tasks.map((t) => `
        <div class="task">
        ...
        </div>`).join("") : `<div class="empty">No tasks assigned yet</div>`;
      return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8"/>
...
var tasksProvider_1 = require_tasksProvider();
var filesProvider_1 = require_filesProvider();
var siloFs_1 = require_siloFs();
var loginPanel_1 = require_loginPanel();
var welcomePanel_1 = require_welcomePanel();
async function activate(context) {
  ...
  let _knownTaskIds = /* @__PURE__ */ new Set();
  let _currentMe = null;
  function updateStatusBar(me) {
  ...
  async function refreshSession(token, refreshFiles = true) {
    try {
      ...
      updateStatusBar(me);
      welcomePanel_1.WelcomePanel.update(me);
      return me;
    } catch {
      vscode.window.showErrorMessage("SimmSilos: failed to load profile");
    }
  }
  async function boot(refreshFiles = false) {
  ...
  async function pollTasks() {
    ...
    try {
      ...
      newTasks.forEach((t) => {
        vscode.window.showInformationMessage(`SimmSilos: new task \u2192 ${t.function} on ${t.branch}`, "View Tasks").then((a) => {
          if (a === "View Tasks")
  ...
  function registerMarkCommand(command, status) {
    return vscode.commands.registerCommand(command, async (item) => {
  ...
  context.subscriptions.push(vscode.commands.registerCommand("simmsilos.login", () => loginPanel_1.LoginPanel.show(context, doLogin)), vscode.commands.registerCommand("simmsilos.logout", async () => {
    ...
    const confirm = await vscode.window.showWarningMessage("Sync will overwrite your local silo files. Continue?", "Yes", "No");
    if (confirm !== "Yes")
      return;
    await api.syncSilo(token);
    filesProvider.refresh();
    vscode.window.showInformationMessage("Silo synced from project");
  }), registerMarkCommand("simmsilos.markDone", "done"), registerMarkCommand("simmsilos.markInProgress", "in_progress"), vscode.commands.registerCommand("simmsilos.viewTask", async (task) => {
    const panel = vscode.window.createWebviewPanel("simmsilosTask", `Task: ${task.function}`, vscode.ViewColumn.One, {});
    const date = new Date(task.assigned_at * 1e3).toLocaleString();
    ...
    const me = _currentMe;
    if (!me?.branch)
      return vscode.window.showInformationMessage("No branch assigned");
    vscode.window.showInformationMessage(`Current branch: ${me.branch}. Contact admin to switch.`);
  }), vscode.commands.registerCommand("simmsilos.diffSilo", async (item) => {
    const token = await auth.ensureFreshToken(context);
    if (!token)
      return;
    const path = item?.resourcePath;
    ...
    try {
      ...
      vscode.commands.executeCommand("vscode.diff", beforeUri, (0, siloFs_1.siloUri)(path), `Project master \u2194 Silo: ${path}`);
    } catch {
  ...
  if (!existingToken) {
    ...
    if (initial) {
      ...
      welcomePanel_1.WelcomePanel.show(context, initial);
    }
  }
}
function deactivate() {
